...
function List() {
  ...
  return (
    <div className="to-do-list">
      ...
      <div>
      {tasks.length < 1 ? <h3 style={{color: "white"}}>Please Add the Task</h3>:
        <div className="w-100">
          {sortedTasks.map((taskItem, index) => (
            <div className="li" key={taskItem.id}>
              <span className="text">{taskItem.name}</span>
          ...
          <div className="mve-btn" onClick={() => moveTaskUp(taskItem.id)}>
  {index !== 0 && <ArrowUpwardIcon  />}
</div>
<div className="mve-btn" onClick={() => moveTaskDown(taskItem.id)}>
  {index !== sortedTasks.length - 1 && <ArrowDownwardIcon  />}
</div>
